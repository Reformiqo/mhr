...
def calculate_received_totals(doc, method=None):
    """MI1-I133 follow-up (Raj 2026-09-10): Received Total Qty / Received
    Total Cone are a plain sum over THIS document's own Item rows that carry
    a Target Warehouse — `custom_received_total_qty` = SUM(qty),
    `custom_received_total_cone` = SUM(custom_cone), both restricted to rows
    where `t_warehouse` is set. A row's own Source Warehouse, if also set
    (an ordinary single-pair Material Transfer row), does not disqualify it
    — only a pure source-only row (Source Warehouse set, Target Warehouse
    blank) is excluded. For both HTY and VFY; nothing else on the document
    is read or changed.

    Round 1 (shipped 2026-09-09) read the live Serial and Batch Bundle
    balance of a separately-picked Received Item in the header's Default
    Target Warehouse instead — correct by that literal FRD wording, but
    wrong in practice: a still-draft receipt has not posted anything to the
    ledger yet, so a first-time item's balance read 0 even though the row
    the user had just entered plainly carried Qty and Cone
    (MAT-GD-2026-00016). `custom_received_item` stays as its own field
    (still the FRD's "pick which item this is" control) but no longer feeds
    ...
    for row in doc.get("items") or []:
        if not row.get("t_warehouse"):
            continue
        qty += flt(row.get("qty"))
        cone += cint(row.get("custom_cone"))
    doc.custom_received_total_qty = qty
