...
class TestBatchFetchFromParity(FrappeTestCase):
	...
	def test_shared_fields_keep_their_position(self):
		...
		'Sales Order Booking' flow writes to. They gain only fetch_from; their
		insert_after must stay on the Details tab, or the VFY form rearranges."""
		import json
		...
		expected_anchor = {
			"custom_container_no": "custom_daniar",
			"custom_lot_no": "custom_container_no",
			"custom_cone": "custom_lot_no",
		}
		for fieldname, anchor in expected_anchor.items():
			with self.subTest(fieldname=fieldname):
				record = records.get(f"Sales Order-{fieldname}")
				self.assertIsNotNone(record)
				self.assertEqual(
					record["insert_after"], anchor,
					"This field moved off the Details tab; the VFY form depends on it.",
				)
				self.assertTrue(record["fetch_from"])
...
class TestFieldsShipAsFixtures(FrappeTestCase):
	"""The HTY fields live in mhr/fixtures/custom_field.json, not in a patch.

	A patch runs once: frappe records the whole patches.txt line in Patch Log
	and skips anything already there (frappe/modules/patch_handler.py ::
	executed), so every later edit is a silent no-op on exactly the sites that
	already migrated. That is how the fetch_from declarations, custom_cone_copy
	and custom_qty_manual_edit all failed to reach a migrated site.

	sync_fixtures() runs on EVERY migrate, so the fixture cannot drift.
	"""
	...
	def test_no_patch_creates_sales_order_hty_fields(self):
		path = frappe.get_app_path("mhr", "patches.txt")
		with open(path, encoding="utf-8") as f:
			self.assertNotIn(
				"add_sales_order_hty_fields", f.read(),
				"The fields ship as fixtures now; a patch would only run once.",
			)

	def test_custom_field_is_exported_as_a_fixture(self):
		from mhr import hooks

		doctypes = [
			entry["doctype"] if isinstance(entry, dict) else entry for entry in hooks.fixtures
		]
		self.assertIn("Custom Field", doctypes)

	def test_every_hty_field_is_in_the_fixture(self):
		records = self._fixture()
		for fieldname in (
			"custom_hty_tab", "custom_batch", "custom_denier", "custom_glue",
			"custom_product", "custom_pulp", "custom_type", "custom_lusture",
			"custom_colour", "custom_grade", "custom_fsc", "custom_merge_no",
			"custom_cross_section", "custom_supplier_batch_no", "custom_scan_batch_no",
			"custom_warehouse", "custom_notes", "custom_count",
			"custom_fetch_batches", "custom_total_cone",
		):
			with self.subTest(fieldname=fieldname):
				self.assertIn(f"Sales Order-{fieldname}", records)

		for fieldname in ("custom_cone_copy", "custom_qty_manual_edit", "custom_sr_no", "custom_gross_weight"):
			with self.subTest(fieldname=fieldname):
				self.assertIn(f"Sales Order Item-{fieldname}", records)

	def test_fields_carry_the_mhr_module(self):
		"""hooks.fixtures filters on module == Mhr. A field without it is
		exported by nobody and vanishes on the next export."""
		records = self._fixture()
		for name, record in records.items():
			if record["dt"] in ("Sales Order", "Sales Order Item"):
				with self.subTest(name=name):
					self.assertEqual(record["module"], "Mhr")


class TestHTYTabIsLastOnTheForm(FrappeTestCase):
	"""A Tab Break claims every field after it until the next Tab Break.

	Anchored at `more_info`, the HTY tab swallowed Status, Commission and Auto
	Repeat — they render under HTY and the More Info tab is left empty.
	Anchoring it at the last field before `connections_tab` puts it between
	More Info and Connections, so it holds only its own fields.
	"""

	def test_tab_sits_immediately_before_connections(self):
		import json
		...
		self.assertEqual(
			records["Sales Order-custom_hty_tab"]["insert_after"],
			"party_account_currency",
			"Anchored anywhere earlier, the HTY tab swallows the standard "
			"sections that follow it.",
		)

	def test_the_anchor_is_still_the_last_field_before_connections(self):
		"""If ERPNext adds a field after party_account_currency, that field
		lands inside the HTY tab and the anchor needs moving."""
		meta = frappe.get_meta("Sales Order")
		fieldnames = [df.fieldname for df in meta.fields]

		if "connections_tab" not in fieldnames:
		...
		self.assertEqual(
			...
			"Sales Order-custom_hty_tab in custom_field.json.",
		)

	def test_no_standard_section_follows_the_tab(self):
		"""The three that were being swallowed."""
		meta = frappe.get_meta("Sales Order")
		fieldnames = [df.fieldname for df in meta.fields]
		if "custom_hty_tab" not in fieldnames:
			self.skipTest("HTY tab not installed on this site.")

		after_tab = fieldnames[fieldnames.index("custom_hty_tab") + 1 :]
		if "connections_tab" in after_tab:
			after_tab = after_tab[: after_tab.index("connections_tab")]

		for fieldname in after_tab:
			with self.subTest(fieldname=fieldname):
				self.assertTrue(
					fieldname.startswith("custom_hty_")
					or frappe.db.exists(
						"Custom Field", {"dt": "Sales Order", "fieldname": fieldname, "module": "Mhr"}
					),
					f"{fieldname} is a standard Sales Order field rendering under "
					"the HTY tab.",
				)
